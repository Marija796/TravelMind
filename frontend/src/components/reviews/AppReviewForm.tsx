...
import { createAppReview, updateAppReview } from '@/services/appReviews'
...
interface Props {
  ...
  onSuccess: (review: AppReview) => void
}

export default function AppReviewForm({ existingReview, onSuccess }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      const review = isEditing ? await updateAppReview(payload) : await createAppReview(payload)
      onSuccess(review)
      if (!isEditing) {
        reset()
        setRating(0)
      }
      toast.success(isEditing ? t('appReviews.updated') : t('appReviews.submitted'))
    } catch (err: unknown) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-5 space-y-4 border border-slate-100 dark:border-slate-700">
      <h3 className="font-semibold text-slate-900 dark:text-white">
        {isEditing ? t('appReviews.editReview') : t('appReviews.writeReview')}
      </h3>
      <div>
      ...
      <Button type="submit" isLoading={isSubmitting} leftIcon={<Send className="w-4 h-4" />}>
        {isEditing ? t('appReviews.updateReview') : t('review.submit')}
      </Button>
    </form>
  )
}
