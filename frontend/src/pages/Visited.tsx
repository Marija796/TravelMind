  ...
  return (
    <div className="pt-24 pb-16 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{t('visited.title')}</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">{t('visited.subtitle')}</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl">
            ...
            <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">{t('visited.empty')}</h3>
            <p className="text-slate-500 dark:text-slate-400 mb-6">{t('visited.emptyHint')}</p>
            <Button onClick={() => navigate('/explore')}>{t('visited.explore')}</Button>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {items.map((dest) => (
                <div key={dest.id} className="relative group">
                  <DestinationCard destination={dest} isVisited />
                  <button
